...
class Collection extends Component
{
    ...
    function get_profit($type) {
        $profit        = 0;
        $getReceipts   = receiptCollection::where('status', '=', 2);
        if ($type == 'month') {
            ...
            foreach(json_decode($receipt->bills_data) as $data) {
                ...
                if(json_decode($data->item_data)->model == \App\Models\Product::class) {
                  // Start Buy Product
                    ...
                    if ($product != null) {
                        $Bill               = billsCollection::where('id', '=', $data->bill_id)->first();
                        if ($Bill != null) {
    ...
    public function render()
    {
        ...
        $getReceipts   = receiptCollection::where('status', '=', 2)->with('client:id,name,email')->whereYear('created_at', Carbon::now()->year)->orderBy('created_at', 'DESC')->paginate(20);
        $receipts = [];
        ...
        return view('livewire.admin.profet.collection', [
