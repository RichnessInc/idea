            ...
            <div class="shadow bg-white p-3 rounded">
                            ...
                            @foreach(json_decode($row->bills_data) as $data)
                                @if(json_decode($data->item_data)->model != \App\Models\Product::class)
                                    <tr class="flex flex-col flex-no wrap sm:table-row mb-2 sm:mb-0 bg-white dark:bg-gray-900 mob-text-aline">
                                        <td class="border-grey-light border dark:text-gray-300 dark:hover:bg-gray-800 hover:bg-gray-100 p-3">ادارة المنصة</td>
                                        <td class="border-grey-light border dark:text-gray-300 dark:hover:bg-gray-800 hover:bg-gray-100 p-3">{{json_decode($data->item_data)->name}}</td>
                                        <td class="border-grey-light border dark:text-gray-300 dark:hover:bg-gray-800 hover:bg-gray-100 p-3">1</td>
                                        <td class="border-grey-light border dark:text-gray-300 dark:hover:bg-gray-800 hover:bg-gray-100 p-3">{{number_format($data->item_price, 2)}} SAR</td>
                                    </tr>
                                @else
                                    <tr class="flex flex-col flex-no wrap sm:table-row mb-2 sm:mb-0 bg-white dark:bg-gray-900 mob-text-aline">
                ...
                <div class="mx-1 richness-table clients-table">
                    <table class="w-full flex flex-row flex-no-wrap sm:bg-white sm:dark:bg-gray-900 rounded-lg overflow-hidden sm:shadow-lg my-5">
                        ...
                        <tbody class="flex-1 sm:flex-none">
                            <tr class="flex flex-col flex-no wrap sm:table-row mb-2 sm:mb-0 bg-white dark:bg-gray-900 mob-text-aline">
                                <td class="border-grey-light border dark:text-gray-300 dark:hover:bg-gray-800 hover:bg-gray-100 p-3">{{number_format($row->total_price - $row->total_shipping)}} SAR</td>
                                ...
                                <td class="border-grey-light border dark:text-gray-300 dark:hover:bg-gray-800 hover:bg-gray-100 p-3">{{number_format($row->total_price, 2)}} SAR</td>
        ...
        <script>
            ...
            setTimeout(() => {
                ...
                $.ajax({
                    ...
                    url: window.location.origin+'/update-receipts-for-user/'+"{{$row->id}}",
                    data: {
