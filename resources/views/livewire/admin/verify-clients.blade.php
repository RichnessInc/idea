<div>
    ...
    <x-jet-dialog-modal wire:model="banFormVisible">
        <x-slot name="title">{{ __('الملفات') }}</x-slot>
        <x-slot name="content">
            <div class="mt-4">
                <div class="images">
                    @if($myFiles != null)
...
<!-- This example requires Tailwind CSS v2.0+ -->
            ...
            @foreach ($users as $user)
                <tr class="flex flex-col flex-no wrap sm:table-row mb-2 sm:mb-0 bg-white dark:bg-gray-900 mob-text-aline">
                    ...
                    <td class="border-grey-light border dark:text-gray-300 dark:hover:bg-gray-800 hover:bg-gray-100 p-3">
                        ...
                        <x-jet-button wire:click="banFun({{$user->id}})" class="main-btn mb-1 w-full editBtn"><i class="mr-2 fas fa-files"></i> {{__(' الملفات ')}}</x-jet-button>
                    </td>
                </tr>
            @endforeach
            </tbody>
    ...
    <br>
    <div class="mx-1">{{$users->links()}}</div>
    @push('scripts')
        <script>
