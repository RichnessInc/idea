<div>
    ...
    <x-jet-dialog-modal wire:model="updateFormVisible" class="">
        <x-slot name="title">{{ __('تعديل بيانات الريشة') }}</x-slot>
        <x-slot name="content">
            <hr style="border-color:#000 !important">
            <div class="mt-4">
                <x-jet-label for='price' value="{{__('السعر')}}" />
    ...
    <x-jet-dialog-modal wire:model="statusFormVisible">
        <x-slot name="title">{{ __('تغير حالة ريشة المرح') }}</x-slot>
        <x-slot name="content">{{ __('انت متأكد من قرار تغير حالة ريشة المرح') }}</x-slot>
        <x-slot name="footer">
