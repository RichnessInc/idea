            ...
            <div class="mx-1 md:mx-0 clients-counter grid grid-cols-12 gap-4 mb-6">
                <div class="col-span-12 md:col-span-12">
                    <div class="rounded overflow-hidden shadow-lg bg-white w-full border-2 border-blue-400">
                        <div class="px-6 py-4 w-full text-center">
                            <div class="mt-4">
                                <h3 class="mb-3"> القيمة الحاليه {{number_format(\App\Models\Readbox::findOrFail(1)->readbox_cost, 2)}} SAR</h3>
                                <x-jet-label for='name' value="{{__('قيمة توصيل ReadBox')}}" />
                                <x-jet-input type='text' id="readbox_cost" wire:model='readbox_cost' class="block mt-1 w-full" />
                                @error('readbox_cost')
                                <span class="text-red-500 mt-1">{{$message}}</span>
                                @enderror
                            </div>
                            <div class="mt-4">
                               <x-jet-button wire:click="change" class="w-full block">تغير</x-jet-button>
                            </div>
                            <br>
                            <hr>
                            <br>
                            <h3 class="text-xl">مستحقات ReadBox</h3>
                            <span class="text-2xl text-blue-400 font-extrabold"  style="direction: ltr;display:block;">
                                 {{number_format(\App\Models\Readbox::findOrFail(1)->dues, 2)}} SAR</span>
                            <br>
                            <hr>
                            <br>
                            <div class="mt-4">
                                <x-jet-button wire:click="zero" class="w-full block">تسديد</x-jet-button>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        @foreach ($getReceipts as $bill)
                            <tr class="flex flex-col flex-no wrap sm:table-row mb-2 sm:mb-0 bg-white dark:bg-gray-900 mob-text-aline">
                                ...
                                <td class="border-grey-light border dark:text-gray-300 dark:hover:bg-gray-800 hover:bg-gray-100 p-3">
                                    <a href="{{URL::to('receipts-for-user/'.$bill['id'])}}"
