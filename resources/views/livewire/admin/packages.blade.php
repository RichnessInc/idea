<div>
    <div class="search-container grid grid-cols-12 gap-4">
        ...
        <div class="col-span-2">
            <x-jet-button  wire:click='createShowModel' class="mb-2 w-full h-10 text-center main-btn">
                <i class="fas fa-plus"></i> {{__('اضافة باقة')}}
            </x-jet-button>
        </div>
    </div>
    ...
    <x-jet-dialog-modal wire:model="createFormVisible">
        <x-slot name="title">{{__('اضافة باقة')}}</x-slot>
        <x-slot name="content">
            ...
            <div class="mt-4">
                <x-jet-label for='description-add' value="{{__(' الوصف')}}" />
                <textarea id="description-add" wire:model='description' class="block mt-1 w-full"></textarea>
                @error('description')
                <span class="text-red-500 mt-1">{{$message}}</span>
                @enderror
            </div>
    ...
    <x-jet-dialog-modal wire:model="updateFormVisible" class="">
        <x-slot name="title">{{ __('تعديل بيانات باقة') }}</x-slot>
        <x-slot name="content">
            <hr style="border-color:#000 !important">
            <div class="mt-4">
            ...
            <div class="mt-4">
                <x-jet-label for='description' value="{{__(' الوصف')}}" />
                <textarea id="description" wire:model='description' class="block mt-1 w-full"></textarea>
                @error('description')
                <span class="text-red-500 mt-1">{{$message}}</span>
                @enderror
            </div>
    ...
    @if (Auth::user()->role == 1)
        <x-jet-dialog-modal wire:model="deleteFormVisible">
            <x-slot name="title">{{ __('حذف باقة') }}</x-slot>
            <x-slot name="content">{{ __('انت متأكد من قرار حذف باقة') }}</x-slot>
            <x-slot name="footer">
        ...
        <table class="w-full flex flex-row flex-no-wrap sm:bg-white sm:dark:bg-gray-900 rounded-lg overflow-hidden sm:shadow-lg my-5">
            ...
            @foreach ($rows as $country)
                <tr class="flex flex-col flex-no wrap sm:table-row mb-2 sm:mb-0 bg-white dark:bg-gray-900 mob-text-aline">
                    ...
                    <td class="border-grey-light border dark:text-gray-300 dark:hover:bg-gray-800 hover:bg-gray-100 p-3">{{number_format($country->price, 2)}} SAR</td>
                    <td class="border-grey-light border dark:text-gray-300 dark:hover:bg-gray-800 hover:bg-gray-100 p-3">{{$country->buyers_count}}</td>
