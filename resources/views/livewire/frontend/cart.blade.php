        ...
        <table class="w-full flex flex-row flex-no-wrap sm:bg-white sm:dark:bg-gray-900 rounded-lg overflow-hidden sm:shadow-lg my-5">
            <thead class="text-white mob-text-aline">
                ...
                <tr class="flex flex-col flex-no wrap sm:table-row rounded-l-lg sm:rounded-none mb-2 sm:mb-0 table-tr">
                    ...
                    <th class="p-3"><i class="fas fa-shipping-fast"></i>  الشحن</th>
            ...
            <tbody class="flex-1 sm:flex-none">
                @foreach ($bills as $bill)
                    @if(json_decode($bill->item_data)->model == \App\Models\Gift::class ||
                        json_decode($bill->item_data)->model == \App\Models\Reesh::class ||
                         json_decode($bill->item_data)->model == \App\Models\Package::class ||
                        json_decode($bill->item_data)->model == \App\Models\GiftTicket::class)
                        <tr class="flex flex-col flex-no wrap sm:table-row mb-2 sm:mb-0 bg-white dark:bg-gray-900 mob-text-aline">
                            <td class="border-grey-light border dark:text-gray-300 dark:hover:bg-gray-800 hover:bg-gray-100 p-3">{{json_decode($bill->item_data)->name}}</td>
                            ...
                            <td class="border-grey-light border dark:text-gray-300 dark:hover:bg-gray-800 hover:bg-gray-100 p-3">{{number_format($bill->shipping, 2)}} SAR</td>
                    ...
                    @elseif(json_decode($bill->item_data)->model == \App\Models\Product::class)
                        <tr class="flex flex-col flex-no wrap sm:table-row mb-2 sm:mb-0 bg-white dark:bg-gray-900 mob-text-aline">
                            ...
                            paypal.Buttons({
                                ...
                                onApprove: function(data, actions) {
                                    return actions.order.capture().then(function(details) {
                                        ...
                                        getAmountRequest.open('GET', location.origin + '/paylink/get-amount', true);
...
//
                            ...
                            {{--
                                                    <x-jet-button wire:click="withPayPal({{$total - $ticketValue}})" class=" w-full ml-2 paymentmethod paypal"><img src="{{asset('images/icons/paypal.png')}}" alt=""> <b>{{__('دفع بباي بال')}}</b></x-jet-button>
                ...
                @elseif ($paymentmethods->id == 3)
                    <div class="col-span-12 lg:col-span-3">
                        ...
                        @endphp
                        <x-jet-button class="paymentmethod visa shadow" style="padding: 0; margin: 0; background-color: transparent; border-radius: 0.25rem; overflow: hidden; width: 292px; height: 50px;" data-info="{{json_encode($arr)}}">
